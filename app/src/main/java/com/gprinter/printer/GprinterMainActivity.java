...
public class GprinterMainActivity extends Activity {
    ...
    public void getPrinterStatusClicked(View view) {
        try {
            ...
            Toast.makeText(getApplicationContext(),
                    "打印机：" + mPrinterIndex + " 状态：" + str, Toast.LENGTH_SHORT).show();
        } catch (RemoteException e1) {
            // TODO Auto-generated catch block
            e1.printStackTrace();
        }
...
//		esc.addPrintAndLineFeed();
        ...
        int rel;
        try {
            rel = mGpService.sendEscCommand(id, str);
            GpCom.ERROR_CODE r = GpCom.ERROR_CODE.values()[rel];
            if (r != GpCom.ERROR_CODE.SUCCESS) {
                Toast.makeText(getApplicationContext(), GpCom.getErrorText(r),
                        Toast.LENGTH_SHORT).show();
    ...
    void sendReceiptBmp(int i) {
		...
		/*打印图片*/
        esc.addText("Print bitmap!\n");   //  打印文字
        Bitmap b = BitmapFactory.decodeResource(getResources(), R.mipmap.test);
    ...
    public void printTest1Clicked(View view) {
        try {
            int type = mGpService.getPrinterCommandType(mPrinterIndex);
            if (type == GpCom.ESC_COMMAND) {
                ...
                mTotalCopies1 += copies;
                for (int i = 0; i < copies; i++) {
                    sendReceipt(i, 0);
                }

                Toast.makeText(getApplicationContext(),
                        "Printer 001 已打印 " + mTotalCopies1 + " 份", Toast.LENGTH_SHORT).show();
            }
        } catch (RemoteException e1) {
            // TODO Auto-generated catch block
            e1.printStackTrace();
    ...
    public void printTest2Clicked(View view) {
        try {
            ...
            if (type == GpCom.ESC_COMMAND) {
                ...
                mTotalCopies2 += copies;
                for (int i = 0; i < copies; i++) {
                    sendReceipt(i, 1);
                }
                Toast.makeText(getApplicationContext(),
                        "Printer 002 已打印 " + mTotalCopies2 + " 份", Toast.LENGTH_SHORT).show();
    ...
    public void printTest3Clicked(View view) {
        try {
            int type = mGpService.getPrinterCommandType(2);
            if (type == GpCom.ESC_COMMAND) {
                EditText etCopies = (EditText) findViewById(R.id.etPrintCopies);
                String str = etCopies.getText().toString();
                int copies = 0;
                ...
                mTotalCopies3 += copies;
                for (int i = 0; i < copies; i++) {
                    sendReceipt(i, 2);
                }
                Toast.makeText(getApplicationContext(),
                        "Printer 003 已打印 " + mTotalCopies3 + " 份", Toast.LENGTH_SHORT).show();
